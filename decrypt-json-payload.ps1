...
function Decrypt-Field {
    ...
    if (![string]::IsNullOrEmpty($IVBase64)) {
        ...
        Write-Host "IV is missing. Skipping decryption."
...
try {
    ...
    foreach ($field in $fieldsToDecrypt) {
        ...
        foreach ($credential in $jsonObject.$firstobject.credentials) {
            if ($credential.$field) {
                $decryptedValue = Decrypt-Field -encryptedValueBase64 $credential.$field.EncryptedValue -IVBase64 $credential.$field.IV
                $credential.$field = $decryptedValue
            }
        }
    }
